...
import com.antmendoza.temporal.HelloActivity;
...
class Worker {
    ...
    static final String TASK_QUEUE = "my-child-task";
...
//        System.out.println(">>> " + client.getWorkflowServiceStubs().healthCheck().getStatus());
    ...
    private static void createWorker(WorkerFactory factory, String taskQueue) {
        io.temporal.worker.Worker worker = factory.newWorker(taskQueue, WorkerOptions.newBuilder()
                .build());

        worker.registerWorkflowImplementationTypes(ChildWorkflowImpl.class, Parent1Impl.class,  Parent2Impl.class);
    }
}
